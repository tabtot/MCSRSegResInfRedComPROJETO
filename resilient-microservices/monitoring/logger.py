...
# Criar diretório de logs e ficheiro inicial
Path(LOGS_DIR).mkdir(parents=True, exist_ok=True)
if not os.path.exists(LOGS_FILE):
    with open(LOGS_FILE, 'w') as f:
        json.dump([], f)
    print(f"Created empty log file: {LOGS_FILE}")
...
def write_log(log_entry):
    try:
        ...
        print(f"Log written: {log_entry.get('service')} - {log_entry.get('message')}")
        return True
